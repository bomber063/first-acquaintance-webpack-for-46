!function () {
    ...
    var controller = Controller({
        ...
        messageList: null,
        myForm: null,
        init: function () {//这里把view和model参数省略也没有关系
            // this.view=view
            // this.model=model
        ...
        loadMessages: function () {
            this.model.fetch()
                .then(//这里的第一个函数是前面的find没有问题会执行第一个函数，如果有问题就执行第二个函数
                    function (messages) {
                        ...
                        array.forEach((item) => {
                            ...
                            // li.textContent = item.name+':'+item.content//这里用innerText也可以
                    ...
                    function (error) {//如果有问题就执行这个函数
                        alert('提交失败，请改天留言')
                    })
                .then(
                    (s) => { console.log(s) },//这里的函数是前面的then后面两个函数都没有问题才会执行这个函数。
                    (error) => { console.log(error) }//这里的函数是前面的then后面两个函数如果有一个有问题就会执行这个函数，比如前面的#messageList，把#删除就会执行该函数，因为有错误
                );
        },
        bindEvents: function () {//这里不能用箭头函数，用了箭头函数会报错，因为箭头函数没有自己的this
            this.myForm.addEventListener('submit', (e) => {//绑定事件里面的代码必须用箭头函数才不会改变this。
