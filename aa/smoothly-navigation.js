!function () {
  ...
  controller = {
    ...
    scrollToElement: function (x) {
      //这里是把66行代码的element传入进来啦
      let top = x.offsetTop; // let n = 20//一共动了多少次
      // let t = 500/n//多少时间动一次
      ...
      var tween = new TWEEN.Tween(coords).to({
        x: 0,
        y: targetTop
      }, t).easing(TWEEN.Easing.Quadratic.In).onUpdate(function () {
        window.scrollTo(coords.x, coords.y); //     box.style.setProperty('transform', 'translate(' + coords.x + 'px, ' + coords.y + 'px)');

        console.log(coords.x, coords.y);
      }).start(); //增加了tween.js库后的缓动平滑过渡的跳转页面
    },
    bindEvents: function () {
      var _this = this;
      ...
      for (i = 0; i < aTags.length; i++) {
        aTags[i].onclick = function (x) {
          ...
          _this.scrollToElement(element); //把element参数传给18行代码的函数

        };
      }
    }
  };
  controller.init(view); // console.log(as[0].href)//这个的输出结果是http://192.168.1.3:8080/#aboutSite,但是我们只想要#aboutSite
}.call();