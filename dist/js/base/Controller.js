...
window.Controller = function (options) {
  ...
  bindEvents = options.bindEvents; // debugger

  return {
    view: null,
    model: null,
    init: function (view, model) {
      //这里的init是函数的一个属性，首先调用这个init函数，这是第一步
      // debugger
      this.view = view;
      this.model = model;
      this.model.init(); // debugger

      console.dir(this); //这里打出来的this和下面打出来的this结果相同，但是a的调用却不能放到这个地方很奇怪。

      for (key in options) {
        //因为return后面整个包裹起来的对象是没有loadMessages和saveMessage的属性的，所以需要options[key]传给它，因为init已经有了，所以可以排除init在外
        if (key !== 'init') {
          //这里的'init'也可以写成a
